...
# ╔═╡ c4a02848-faea-43a0-8cd4-2ea3d74a4828
using Plots, DifferentialEquations

# ╔═╡ 0c4d0563-c5ab-44d0-9cb3-5471071dffee
md"# Equações de Lorenz"

# ╔═╡ 187175a8-ef28-4946-bc54-4967abbd506f
md"São os sistemas da forma:"

# ╔═╡ 8a0efa4e-84e0-485e-ae73-83671bc71e3b
md"$\begin{align}
...
# ╔═╡ 7e536f46-ac97-43c6-a232-485e8749a7d3
md"""
Vamos simular as trajetórias dos sistemas de 3 maneiras diferentes:
- Método de Euler
...
md"Primeiro vamos definir um tipo de variável Lorenz, a qual contém todos os parâmetros que precisaremos para realizar a simulação"

# ╔═╡ c3316c07-bb4e-409d-9183-a8090e3ea131
@kwdef mutable struct Lorenz
    dt::Float64 = 0.01 #Tamanho do passo
    σ::Float64 = 10 
    r::Float64 = 28
    b::Float64 = 8/3
    x::Float64 = 2 #coordenada inicial x
    y::Float64 = 1 #      ""           y
    z::Float64 = 1 #      ""           z
end;
...
# ╔═╡ c792b540-d49d-11f0-97d3-130acd150530
md"# Método 2: Runge-Kutta 4 (RK4)"

# ╔═╡ 0cfd7d0c-a496-42f8-acd7-ab88a9dd840b
md"Iremos utilizar a mesma estrutura Lorenz usada no método anterior, portantod não precisaremos defini-la denovo."

# ╔═╡ a870182e-5aa2-4515-9fe7-d0c7aa39bc28
md"Como para o método RK4 precisaremos calcular iterativamente o valor das derivadas, teremos que definir uma função separada para tal:"

# ╔═╡ 9043da54-6c7f-4d33-985b-a7bd6cc7350c
function lorenz_rk(l::Lorenz, x, y, z)
    dx = l.σ * (y - x)
    dy = x * (l.r - z) - y
    dz = x * y - l.b * z
    return (dx, dy, dz)
...
md"Com isso feito, podemos criar a função step_rk! para calcular o nosso proximo ponto da trajetória."

# ╔═╡ 343fafa3-90d5-49db-a009-784765ccc6ae
function step_rk!(l::Lorenz)
    dt = l.dt     
    # k1: Inclinação no início do intervalo
    k1x, k1y, k1z = lorenz_rk(l, l.x, l.y, l.z)
    
    # k2: Inclinação no ponto médio (usando k1)
    k2x, k2y, k2z = lorenz_rk(l, l.x + 0.5*dt*k1x, l.y + 0.5*dt*k1y, l.z + 0.5*dt*k1z)
    
    # k3: Outra inclinação no ponto médio (usando k2)
    k3x, k3y, k3z = lorenz_rk(l, l.x + 0.5*dt*k2x, l.y + 0.5*dt*k2y, l.z + 0.5*dt*k2z)
    
    # k4: Inclinação no final do intervalo (usando k3)
    k4x, k4y, k4z = lorenz_rk(l, l.x + dt*k3x, l.y + dt*k3y, l.z + dt*k3z)
    
    # Atualização final: Média ponderada das inclinações
    l.x += (dt / 6) * (k1x + 2*k2x + 2*k3x + k4x)
    l.y += (dt / 6) * (k1y + 2*k2y + 2*k3y + k4y)
    l.z += (dt / 6) * (k1z + 2*k2z + 2*k3z + k4z)
end;

# ╔═╡ eb8eaa6e-e9f5-48c2-9dcc-59395bf38364
md"O que resta agora é gerar o plot:"
...
begin
	...
	plt_rk = plot3d(
	    ...
	    marker = 1,      # Marcador menor para ficar mais elegante
	    linewidth = 1.5, # Linha um pouco mais fina
	    legend = false
	)
end;

# ╔═╡ 3c00ee63-3114-462b-95ba-98393f0575ff
md"### Está aqui o .gif gerado utilizando o método RK4!"

# ╔═╡ 72db1b68-1992-4fac-afc8-c048b9850318
@gif for i=1:2000
    step_rk!(attractor_rk)
    push!(plt_rk, attractor_rk.x, attractor_rk.y, attractor_rk.z)
end every 10

# ╔═╡ 7161c044-6268-4a54-9535-e1d2bf3b709c
md"# Método 3: DifferentialEquations.jl"

# ╔═╡ b5d3cbbf-c4bc-4a05-999f-bbda1c87b8b1
md"Agora iremos utilizar o .pkg DifferentialEquations e seus métodos para resolver e o sistema e construir o caminho. Como as funções são nativas do pacote, o código será bem mais sucinto."
...
md"Teremos de definir as condições iniciais novamente, mas em outro formato:"
...
md"Vamos definir o problema em uma variável única e utilizar o método solve() do pkg para resover o sistema:"

# ╔═╡ 3cbc0c0d-fe47-4516-a0e2-e81fa0a282ac
begin
	prob = ODEProblem(lorenz_de!, u0, tspan, p);
	sol = solve(prob, saveat=0.01);
end;

# ╔═╡ 773f62b7-e7dc-45ae-976f-d75364b46a91
md"Gerando a animação para o nosso plot:"

# ╔═╡ 9b9daae2-e081-477c-ab51-30377960ee04
anim = @animate for i in 1:5:length(sol)
    plot(sol, idxs=(1, 2, 3), 
         xlim=(-25, 25), ylim=(-30, 30), zlim=(0, 50),
         title="Lorenz Attractor (DiffEq.jl)", 
         alpha=0.5,             
         linewidth=1,
         label=nothing)         
         
    scatter!([sol[i][1]], [sol[i][2]], [sol[i][3]], 
             marker=:circle, color=:blue, label=nothing)
end;

# ╔═╡ 52f1e87c-1abb-493c-acac-82276f95bd8c
md"### Está aqui o nosso último .gif!"

# ╔═╡ 66a679b3-cec8-45ec-b84b-ebc87cfb4730
gif(anim, "lorenz_diffeq.gif", fps=30)
...
manifest_format = "2.0"
project_hash = "bf6a0c968acfaff42203100f98ed7c531e9c65f7"
